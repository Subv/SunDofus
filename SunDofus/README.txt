SunDofus
========

SunDofus is a public and open-sources emulator for Dofus 1.29.1 !
He is written in C#.NET v4.0 by Ghost (Alias Nicolas Petit).

Please use the official database with the adress "./Databases/sunrealm.sql" or "./Databases/sunworld.sql"
and execute the fowolling patch at "./Databases/Patchs/*"

Please send the bugs at nicolas.p.mail@gmail.com

#Ghost.